...
describe('onboarding runtime state-machine E2E', () => {
  ...
  it('completes a stable code flow using event-derived confirmation hashes', async () => {
    ...
    expect(planConfirmation).toMatchObject({ planHash: hashOnboardingPlan({ serverPlanHash: hashServerPlan({ space: { mode: 'create', name: 'E2E Space' }, agentName: 'E2E Agent', role: 'editor', packageVersion: '0.9.0' }), localScanPlanHash: PLAN_A_HASH }) });
