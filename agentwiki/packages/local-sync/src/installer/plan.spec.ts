...
describe('gateway command', () => {
  it('uses the exact pinned 0.3.4 package and gateway subcommand', () => {
    const cmd = gatewayCommand('conn-123');
    expect(cmd).toEqual([
      'npx',
      '--yes',
      '@neomei/agentwiki-local-sync@0.3.4',
      'gateway',
      '--connection',
      'conn-123',
    ]);
  ...
  it('uses the fixed gateway MCP name', () => {
    ...
    expect(GATEWAY_PACKAGE_VERSION).toBe('0.3.4');
  });
});
...
describe('looksLikeAgentWikiEntry', () => {
  ...
  it('does not flag the new gateway name itself', () => {
    expect(looksLikeAgentWikiEntry('agentwiki', 'npx agentwiki-local-sync@0.3.4 gateway')).toBe(false);
  });
