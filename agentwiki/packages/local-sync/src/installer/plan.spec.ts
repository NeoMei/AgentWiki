...
describe('gateway command', () => {
  it('uses the exact pinned 0.3.1 package and gateway subcommand', () => {
  ...
  it('uses the fixed gateway MCP name', () => {
    ...
    expect(GATEWAY_PACKAGE_VERSION).toBe('0.3.1');
  });
});
...
describe('looksLikeAgentWikiEntry', () => {
  ...
  it('does not flag the new gateway name itself', () => {
    expect(looksLikeAgentWikiEntry('agentwiki', 'npx agentwiki-local-sync@0.3.1 gateway')).toBe(false);
  });
