...
```bash
npx --yes @neomei/agentwiki-local-sync@0.8.0 onboard \
...
Every 0.8.0 plan contains one role: `reader`, `editor`, or `publisher`. For an existing
...
The 0.8.0 package requires `@neomei/agentwiki-sync-protocol@0.5.1`, which contains the
collaboration and Sync v3 contracts. Release verification first packs both packages and
installs them into an empty directory. Local Sync 0.8.0 is only a local candidate in this
release plan; publishing it requires later explicit authorization and a separate plan.
