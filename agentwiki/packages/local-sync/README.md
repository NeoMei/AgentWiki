...
```bash
npx --yes @neomei/agentwiki-local-sync@0.7.0 onboard \
...
Every 0.7.0 plan contains one role: `reader`, `editor`, or `publisher`. For an existing
...
The 0.7.0 package requires `@neomei/agentwiki-sync-protocol@0.4.0`, which contains the
...
pass before local-sync 0.7.0 is published.
