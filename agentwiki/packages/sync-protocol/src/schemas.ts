...
import { SYNC_ERROR_CODES } from "./types.js";
...
export const HumanDeviceSessionResponseSchema = z
  .object({
    ...
    provisionalExpiresAt: Rfc3339Schema.nullable(),
    user: z.object({ id: z.string(), displayName: z.string() }),
    capabilities: SyncCapabilitiesSchema,
  })
  .passthrough();

export const HumanDeviceCredentialListResponseSchema = z
  .object({
    protocolVersion: z.literal("1"),
    credentials: z.array(
      z
        .object({
          ...
          vaultId: UuidSchema,
          deviceName: z.string(),
          status: z.enum(["provisional", "active", "revoked", "expired"]),
          provisionalExpiresAt: Rfc3339Schema.nullable(),
          createdAt: Rfc3339Schema,
          lastUsedAt: Rfc3339Schema.nullable(),
...
export const FinalizePushResponseSchema = z
  .object({
    protocolVersion: z.literal("1"),
...
const PushSessionStatusSchema = z.enum([
  "uploading",
  "ready_to_finalize",
  "published",
  "aborted",
...
export const CreatePushSessionResponseSchema = z
  .object({
    protocolVersion: z.literal("1"),
    sessionId: UuidSchema,
    status: PushSessionStatusSchema,
    expiresAt: Rfc3339Schema,
    capabilities: SyncCapabilitiesSchema,
    result: FinalizePushResponseSchema.nullable(),
  })
  .passthrough();
...
export const PushBatchReceiptSchema = z
  .object({
    protocolVersion: z.literal("1"),
    sessionId: UuidSchema,
    batchIndex: z.number().int().nonnegative(),
