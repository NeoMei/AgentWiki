...
export const resolveWikiHref = (reference: WikiReference | string, pages: PageLinkTarget[]): string | null => {
  ...
  const bySlug = pages.find((page) => page.slug && normalizeMarkdownPageIdentity(page.slug) === needle);
  const byTitle = pages.find((page) => page.title && normalizeMarkdownPageIdentity(page.title) === needle);
