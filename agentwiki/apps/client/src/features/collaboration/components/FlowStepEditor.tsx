      ...
      {selected ? (
        <div className="min-w-0 space-y-4 rounded-xl border bg-white p-4">
          ...
          {selected.kind === 'agent_task' ? (
            <AgentTaskForm task={selected} definition={definition} onChange={replaceNode} labels={labels} />
          ) : (
            <ReviewForm node={selected} definition={definition} onChange={replaceNode} labels={labels} />
          )}
...
const AgentTaskForm: React.FC<{ task: AgentTask; definition: CollaborationTemplateDefinition; onChange: (node: Node) => void; labels: Record<string, string> }> = ({ task, definition, onChange, labels }) => {
  ...
  return <>
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      <label className="block text-sm font-medium">{labels.outputKey}<input value={task.output.key} onChange={(event) => onChange({ ...task, output: { ...task.output, key: event.target.value } })} className="mt-1 h-10 w-full rounded-lg border px-3" /></label>
      <label className="block text-sm font-medium">{labels.outputKind}<select value={task.output.kind} onChange={(event) => onChange({ ...task, output: { key: task.output.key, kind: event.target.value as AgentTask['output']['kind'], ...(event.target.value === 'json' ? { jsonSchema: { type: 'object' } } : {}) } })} className="mt-1 h-10 w-full rounded-lg border px-3"><option value="markdown">Markdown</option><option value="json">JSON</option><option value="external_reference">External reference</option><option value="evidence_summary">Evidence summary</option></select></label>
    </div>
    <fieldset className="rounded-lg border p-3"><legend className="px-1 text-sm font-medium">Todo</legend>
...
const ReviewForm: React.FC<{ node: Extract<Node, { kind: 'human_review' }>; definition: CollaborationTemplateDefinition; onChange: (node: Node) => void; labels: Record<string, string> }> = ({ node, definition, onChange, labels }) => {
