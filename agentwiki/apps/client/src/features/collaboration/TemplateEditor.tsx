...
const InputsSection: React.FC<{ definition: CollaborationTemplateDefinition; onChange: (definition: CollaborationTemplateDefinition) => void; t: (key: string) => string }> = ({ definition, onChange, t }) => {
  const rows = useEditableRows(definition.inputs);
  return <section className="space-y-3">
  {definition.inputs.map((input, index) => <div key={rows.key(input)} className="grid gap-3 rounded-xl border bg-white p-4 sm:grid-cols-[1fr_1fr_auto]">
    <label className="text-sm font-medium">{t('collaboration.editor.inputKey')}<input value={input.key} onChange={(event) => onChange({ ...definition, inputs: definition.inputs.map((item, itemIndex) => itemIndex === index ? rows.replace(item, { ...item, key: event.target.value }) : item) })} className="mt-1 h-10 w-full rounded-lg border px-3" /></label>
    <label className="text-sm font-medium">{t('collaboration.editor.inputLabel')}<input value={input.label} onChange={(event) => onChange({ ...definition, inputs: definition.inputs.map((item, itemIndex) => itemIndex === index ? rows.replace(item, { ...item, label: event.target.value }) : item) })} className="mt-1 h-10 w-full rounded-lg border px-3" /></label>
    <div className="flex items-end gap-2"><label className="mb-2 text-sm"><input type="checkbox" checked={input.required} onChange={(event) => onChange({ ...definition, inputs: definition.inputs.map((item, itemIndex) => itemIndex === index ? rows.replace(item, { ...item, required: event.target.checked }) : item) })} /> {t('collaboration.editor.required')}</label><button type="button" aria-label={`${t('common.delete')} ${input.label}`} onClick={() => onChange({ ...definition, inputs: definition.inputs.filter((_, itemIndex) => itemIndex !== index) })} className="mb-1 rounded-lg border border-red-200 p-2 text-red-700"><Trash2 size={15} /></button></div>
    <label className="text-sm font-medium sm:col-span-2">{t('common.type')}<select value={input.type} onChange={(event) => onChange({ ...definition, inputs: definition.inputs.map((item, itemIndex) => itemIndex === index ? rows.replace(item, { ...item, type: event.target.value as typeof input.type }) : item) })} className="mt-1 h-10 w-full rounded-lg border px-3"><option value="short_text">{t('collaboration.editor.inputType.short_text')}</option><option value="long_text">{t('collaboration.editor.inputType.long_text')}</option><option value="number">{t('collaboration.editor.inputType.number')}</option><option value="boolean">{t('collaboration.editor.inputType.boolean')}</option><option value="url">{t('collaboration.editor.inputType.url')}</option></select></label>
  </div>)}
...
const RolesSection: React.FC<{ definition: CollaborationTemplateDefinition; onChange: (definition: CollaborationTemplateDefinition) => void; t: (key: string) => string }> = ({ definition, onChange, t }) => {
  const rows = useEditableRows(definition.roleSlots);
  return <section className="space-y-3">
  {definition.roleSlots.map((role, index) => <div key={rows.key(role)} className="grid gap-3 rounded-xl border bg-white p-4 sm:grid-cols-2">
    <label className="text-sm font-medium">{t('collaboration.editor.roleId')}<input value={role.id} onChange={(event) => onChange({ ...definition, roleSlots: definition.roleSlots.map((item, itemIndex) => itemIndex === index ? rows.replace(item, { ...item, id: event.target.value }) : item) })} className="mt-1 h-10 w-full rounded-lg border px-3" /></label>
    <label className="text-sm font-medium">{t('common.name')}<input value={role.name} onChange={(event) => onChange({ ...definition, roleSlots: definition.roleSlots.map((item, itemIndex) => itemIndex === index ? rows.replace(item, { ...item, name: event.target.value }) : item) })} className="mt-1 h-10 w-full rounded-lg border px-3" /></label>
    <label className="text-sm font-medium sm:col-span-2">{t('common.description')}<input value={role.description} onChange={(event) => onChange({ ...definition, roleSlots: definition.roleSlots.map((item, itemIndex) => itemIndex === index ? rows.replace(item, { ...item, description: event.target.value }) : item) })} className="mt-1 h-10 w-full rounded-lg border px-3" /></label>
    <div className="flex items-center justify-between sm:col-span-2"><label className="text-sm"><input type="checkbox" checked={role.required} onChange={(event) => onChange({ ...definition, roleSlots: definition.roleSlots.map((item, itemIndex) => itemIndex === index ? rows.replace(item, { ...item, required: event.target.checked }) : item) })} /> {t('collaboration.editor.required')}</label>{definition.roleSlots.length > 1 ? <button type="button" aria-label={`${t('common.delete')} ${role.name}`} onClick={() => onChange({ ...definition, roleSlots: definition.roleSlots.filter((_, itemIndex) => itemIndex !== index) })} className="rounded-lg border border-red-200 p-2 text-red-700"><Trash2 size={15} /></button> : null}</div>
  </div>)}
...
// A row's editable business ID is not its React identity. Carry identity across
// immutable field updates; surviving objects retain identity after deletion.
function useEditableRows<T extends object>(items: T[]) {
  const identities = useRef(new WeakMap<T, number>());
  const sequence = useRef(0);
  const key = (item: T) => {
    let value = identities.current.get(item);
    if (value === undefined) { value = ++sequence.current; identities.current.set(item, value); }
    return value;
  };
  items.forEach(key);
  return { key, replace: (previous: T, next: T): T => {
    identities.current.set(next, key(previous));
    return next;
  } };
}
