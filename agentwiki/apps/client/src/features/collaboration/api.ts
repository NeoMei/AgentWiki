...
export const collaborationApi = {
  ...
  listRuns: async (spaceId: string, kind: RunListKind, cursor?: string): Promise<RunListPage> =>
    (await api.get<RunListPage>(`/spaces/${spaceId}/collaboration/runs`, {
      params: { status: kind, limit: 100, ...(cursor ? { cursor } : {}) },
    })).data,
