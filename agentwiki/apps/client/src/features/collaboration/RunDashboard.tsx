...
const ActionDialog: React.FC<{
  ...
  const wasSubmitting = React.useRef(submitting);
  React.useEffect(() => {
    if (wasSubmitting.current && !submitting) reasonRef.current?.focus();
    wasSubmitting.current = submitting;
  }, [submitting]);
  return <ModalDialog labelledBy="collaboration-action-title" onRequestClose={onClose} closeDisabled={submitting} className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl"><h2 id="collaboration-action-title" className="text-lg font-semibold">{label}</h2><p className="mt-2 break-words text-sm text-gray-600">{t('collaboration.dashboard.actionTarget')}: {target}</p>{pending.type === 'review' && pending.kind === 'reject_for_revision' ? <p className="mt-2 text-sm text-gray-600">{t('collaboration.dashboard.returnTask')}: {run.tasks?.find((task) => task.id === pending.review.revisionTaskId)?.name ?? pending.review.revisionTaskId}</p> : null}{pending.type === 'task' && pending.kind === 'reassign' ? <label className="mt-4 block text-sm font-medium">{t('collaboration.dashboard.agent')}<select value={agentId} onChange={(event) => setAgentId(event.target.value)} className="mt-1 h-10 w-full rounded-lg border px-3">{agents.map((agent) => <option key={agent.agentId} value={agent.agentId}>{agent.agent?.name}</option>)}</select></label> : null}<label className="mt-4 block text-sm font-medium">{t('collaboration.dashboard.reason')}<textarea ref={reasonRef} data-modal-autofocus aria-label={t('collaboration.dashboard.reason')} value={reason} onChange={(event) => setReason(event.target.value)} className="mt-1 min-h-24 w-full rounded-lg border p-3" /></label><div className="mt-6 flex justify-end gap-2"><button type="button" disabled={submitting} onClick={onClose} className="min-h-10 rounded-lg border px-4 text-sm">{t('common.cancel')}</button><button type="button" disabled={submitting || !reason.trim() || (pending.type === 'task' && pending.kind === 'reassign' && !agentId)} onClick={onConfirm} className="min-h-10 rounded-lg bg-blue-600 px-4 text-sm font-medium text-white disabled:opacity-50">{t('collaboration.dashboard.confirmAction')} {label.toLowerCase()}</button></div></ModalDialog>;
};
