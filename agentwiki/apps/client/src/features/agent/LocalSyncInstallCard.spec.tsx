...
const instruction = '# 接入\nnpx --yes @neomei/agentwiki-local-sync@0.5.1 onboard --server https://wiki.test/api --code AW-ABCD-EFGH --protocol ndjson --agent auto';
...
const generate = async () => {
  ...
  await screen.findByText(/@neomei\/agentwiki-local-sync@0\.5\.1/);
};
...
describe('LocalSyncInstallCard', () => {
  ...
  it('generates an editor connection intent for the selected Space', async () => {
    ...
    await waitFor(() => expect(api.post).toHaveBeenCalledWith(
      ...
      { pluginVersion: '0.5.1', spaceId: 'space-1', role: 'editor' },
    ));
  });
  ...
  it('moves to an available Space when the selected Space disappears', async () => {
    ...
    await waitFor(() => expect(api.post).toHaveBeenCalledWith(
      '/agents/agent-1/local-sync-installations',
      { pluginVersion: '0.5.1', spaceId: 'space-2', role: 'reader' },
    ));
  });
});
