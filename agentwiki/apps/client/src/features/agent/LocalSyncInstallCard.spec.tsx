...
const instruction = '# 接入\nnpx --yes @neomei/agentwiki-local-sync@0.7.0 onboard --server https://wiki.test/api --code AW-ABCD-EFGH --protocol ndjson --agent auto';
...
const generate = async () => {
  ...
  await screen.findByText(/@neomei\/agentwiki-local-sync@0\.7\.0/);
};
...
describe('LocalSyncInstallCard', () => {
  ...
  it('moves to an available Space when the selected Space disappears', async () => {
    ...
    await waitFor(() => expect(api.post).toHaveBeenCalledWith(
      '/agents/agent-1/local-sync-installations',
      { pluginVersion: '0.7.0', spaceId: 'space-2', role: 'reader' },
    ));
  });
});
