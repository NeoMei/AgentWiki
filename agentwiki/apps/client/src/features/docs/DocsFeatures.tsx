    ...
    <DocsLayout>
      <article className="prose prose-gray max-w-none">
        ...
        <div className="flex justify-between items-center pt-8 mt-8 border-t border-gray-200">
          <Link to="/docs/architecture" className="text-sm text-gray-500 hover:text-gray-700">&larr; {zh ? '系统架构' : 'Architecture'}</Link>
          <Link to="/docs/security" className="text-sm text-blue-600 hover:underline font-medium flex items-center gap-1">{zh ? '安全模型' : 'Security'} <ArrowRight size={14} /></Link>
        </div>
      </article>
    </DocsLayout>
  );
