      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold text-gray-900">AgentWiki</span>
          </div>
          <div className="flex items-center gap-3">
            <LanguageSwitcher />
            {token ? (
              <Link to="/dashboard" className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition">
                {zh ? '进入工作台' : 'Dashboard'}
              </Link>
            ) : (
              <Link to="/guide" className="px-4 py-2 text-sm text-gray-600 hover:text-gray-900 transition">
                {zh ? '使用指南' : 'Guide'}
              </Link>
            )}
          </div>
