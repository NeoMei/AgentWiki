...
test.describe('local sync enrollment card', () => {
  ...
  test('logs in and generates, copies, and shows expiry for a one-shot instruction', async ({ page }) => {
    ...
    await expect(instructions).toContainText('@neomei/agentwiki-local-sync@0.9.1');
    ...
    await expect.poll(() => page.evaluate(() => navigator.clipboard.readText())).toContain('@neomei/agentwiki-local-sync@0.9.1');
