...
const ERROR_CODE_MAP: Record<string, { status: HttpStatus; message: string }> = {
  ...
  TASKBOARD_INVALID: { status: HttpStatus.BAD_REQUEST, message: 'Taskboard input is invalid' },
  TASKBOARD_TASK_NOT_FOUND: { status: HttpStatus.NOT_FOUND, message: 'Taskboard task not found' },
  TASKBOARD_PARENT_NOT_FOUND: { status: HttpStatus.BAD_REQUEST, message: 'Taskboard parent task not found' },
  TASKBOARD_DUPLICATE_ID: { status: HttpStatus.CONFLICT, message: 'Taskboard task id already exists' },
  ...
  FOLDER_NOT_FOUND: { status: HttpStatus.NOT_FOUND, message: 'Folder not found' },
  FOLDER_NAME_CONFLICT: { status: HttpStatus.CONFLICT, message: 'A sibling Folder already uses this name' },
  FOLDER_INVALID_NAME: { status: HttpStatus.BAD_REQUEST, message: 'Folder name is not portable' },
  FOLDER_CYCLE: { status: HttpStatus.CONFLICT, message: 'Folder move would create a cycle' },
  FOLDER_DEPTH_LIMIT: { status: HttpStatus.BAD_REQUEST, message: 'Folder depth limit exceeded' },
  FOLDER_COUNT_LIMIT: { status: HttpStatus.BAD_REQUEST, message: 'Folder count limit exceeded' },
  ...
  CONTENT_TREE_SPACE_FORBIDDEN: { status: HttpStatus.FORBIDDEN, message: 'Space access denied' },
  CONTENT_TREE_SPACE_READ_ONLY: { status: HttpStatus.FORBIDDEN, message: 'This Space membership is read-only' },
  CONTENT_TREE_PAYLOAD_INVALID: { status: HttpStatus.BAD_REQUEST, message: 'Content tree payload is invalid' },
  CONTENT_TREE_PATH_COLLISION: { status: HttpStatus.CONFLICT, message: 'Content tree path conflicts with another item' },
  CONTENT_TREE_ID_CONFLICT: { status: HttpStatus.CONFLICT, message: 'Content tree id belongs to another resource' },
  CONTENT_TREE_TAKE_INVALID: { status: HttpStatus.BAD_REQUEST, message: 'Content tree take is invalid' },
  FOLDER_DELETE_IMPACT_CHANGED: { status: HttpStatus.CONFLICT, message: 'Folder delete impact changed' },
  FOLDER_RESTORE_CONFLICT: { status: HttpStatus.CONFLICT, message: 'Folder restore conflicts with current content' },
  MARKDOWN_REFERENCE_AMBIGUOUS: { status: HttpStatus.CONFLICT, message: 'Markdown reference is ambiguous' },
  PAGE_PARENT_DEPRECATED: { status: HttpStatus.CONFLICT, message: 'Legacy Page parent placement cannot be mapped safely' },
