...
export class AllExceptionsFilter implements ExceptionFilter {
  ...
  constructor(private readonly httpAdapterHost: HttpAdapterHost) {}
    ...
    if (isSyncV3) {
      const businessCode = SyncV3WireErrorCodeSchema.safeParse(getBusinessCode(exception));
      const safeException = exception instanceof SyncApiException
        ? exception
        : new SyncApiException(
          businessCode.success ? businessCode.data : this.syncV3Code(statusCode),
          'Sync v3 request failed',
          undefined,
          '3',
        );
      ...
      const safeStatus = exception instanceof SyncApiException || businessCode.success
