...
本文件记录 sync v1 基线、原 `codex/readable-sync-paths` 分支以及临时最终候选的可追溯验收证据，不替代契约。原分支仍停在 `294b694`；Fix 3A/3B/3C 及 post-completion follow-up 只存在于临时仓候选链，当前产品行为/验证基线 HEAD 为 `961c8eb`，尚未回灌原分支。状态定义：
...
| 56 | 只迁移严格 `pages/p-<64 lowercase hex>.md`，固定批次完成后直接短路，批次幂等且失败全回滚 | Fix 3 前的 `294b694` 曾在真实 PostgreSQL 扩展门禁中通过 32/32；更新后的 2 项 DB 测试当前显式跳过。非 DB 定向套件为 4/4：真实二次 guard 复用同 revision ID；CLI 聚合覆盖首次迁移、已完成 retry、空 Space，只计新 revision；单项与复数汇总分别输出正确的 page path/space/revision 名词 | run/unit |
...
- 可读路径迁移不能再概括为“只有 code/DB skip”：Fix 3 前的 32/32 真实 PostgreSQL 门禁仍是有效历史证据；但它没有执行 Fix 3A 更新后的 fixture 或 post-completion guard/契约/测试真实性/单复数 follow-up。当前 `961c8eb` 的生产 migration/CLI 非 DB 定向套件为 4/4，更新后的 DB 文件仍为 0 pass / 2 explicit skips / 0 fail。
- 临时候选 `961c8eb` 的最新 runtime 为 116 项（75 pass / 41 个无 `DATABASE_URL` 的显式 DB skip / 0 fail）。Controller 在该当前 HEAD fresh 重跑了排除四个 loopback 文件的 root server gate：53 suites / 539 tests pass。Fix 3B 的最后一次完整 server 尝试仍为 57 suites / 577 tests，其中 557 pass，另外四个 HTTP suites / 20 tests 仅因 sandbox `listen 127.0.0.1` 返回 `EPERM` 而失败，未冒充为已通过。
