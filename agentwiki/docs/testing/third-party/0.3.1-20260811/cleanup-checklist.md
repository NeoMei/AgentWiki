...
| Resource type | Sanitized name | Owning user | Space | Creation case | Deletion method | Deletion time | Verifier | Verification method | Result |
|---|---|---|---|---|---|---|---|---|---|
| Device Request handle set (6) | Five authorization-session handles plus the separate consumed-request handle retained by ONBOARD-006 | User A | N/A | ONBOARD-001 / ONBOARD-006 / ONBOARD-007 | Approved-consumed, denied, or expired server outcome; no server deletion required; local session artifacts remain covered by isolated-root cleanup | 2026-08-12T01:18:00+08:00 | Codex controller | Sanitized consumed/denied/expired evidence plus no denied/expired Agent in scoped Agent list | PASS — all six sanitized handles inventoried; local session cleanup remains PENDING |
