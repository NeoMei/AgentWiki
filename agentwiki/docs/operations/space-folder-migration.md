...
The report contains the fixed `snapshotAsOf` timestamp, input snapshot hash, stable per-Page old/new paths, planned Folders and aliases, alias reuse/refresh/pruning and resolution effects, every title transformation, deterministic sibling collision allocation, conversion summaries, and all rejection codes. Existing aliases are part of the reviewed input hash. Resolution ambiguity/current-path shadowing uses only aliases whose owner Page is active and whose expiry is `NULL` or later than this one `snapshotAsOf`; aliases at or before it do not resolve, but aliases that can affect retention/upsert remain hashed. Crossing an expiry boundary between dry-run and apply changes the hash. Legacy sanitization normalizes NFC, folds each consecutive forbidden-character run to one space, collapses whitespace, cleans trailing dots/spaces, handles reserved device names, and hashes any UTF-8 truncation. Preflight is zero-write. Do not apply if it reports a cycle, orphan, cross-Space reference, Folder/path/depth/count/mutation limit, existing-tree inconsistency, invalid alias evidence, or Page/PageVersion placement conflict.
...
Apply repeats preflight after taking the Space advisory lock and rejects `MIGRATION_INPUT_CHANGED` unless the locked input matches the reviewed dry-run hash, including on a completed-key retry. Folder creation, ContentTree-compatible alias upsert/20-entry retention, Page placement/path changes, historical PageVersion placement, the complete first v2 Folder/Page snapshot and deterministic tree delta, the migration revision/batch evidence, and `contentTreeRevision` advancement commit in one transaction. The required apply report target is exclusively created with no-follow/exclusive flags before any database write and remains open as one file descriptor. Final reports use only truncate/write/fsync through that original descriptor; the parent and target device/inode identities are rechecked before the transaction may commit. Successful apply performs exactly one final success write inside the transaction callback and performs no report I/O after commit. If commit itself fails after that callback, the same descriptor is rewritten and fsynced with rejection evidence. Reservation initialization and later failures never unlink, rename, or remove the report pathname; a replaced target or parent therefore cannot be overwritten or deleted. Any pre-commit report persistence failure rolls back the database transaction.
