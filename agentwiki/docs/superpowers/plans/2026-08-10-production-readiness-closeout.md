...
- Produces: npm package and onboarding version `0.2.9`.
...
Add `help` and `version` to `parseArgs` boolean options, set package/onboarding references to `0.2.9`, and keep orchestrator MCP registration enabled.
...
**Interfaces:**
- Produces: npm `0.2.9`, GitHub `master`, production service, and documentation at the same version.
