...
- Every mutation of an existing Run uses `CollaborationRunEvent` scope `(runId, actorKind, actorId, operation, idempotencyKey)` plus a canonical request hash; exact replay returns the stored bounded receipt and mismatched reuse fails. Template and draft updates use optimistic `expectedVersion`; exact create/copy/create-draft DTOs do not participate in RunEvent replay because no `runId` exists yet. Lease-token plaintext is never persisted or logged and may only be deterministically reconstructed for an authorized exact claim replay.
