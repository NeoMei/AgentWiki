...
- 优先完成服务端 OpenCode 模型自动降级与成本路由：免费模型优先、付费显式授权、Redis 共享熔断和 token/cost 记录。
...
- 2026-08-04 OpenCode 模型降级与成本路由的免费优先、客观失败降级、显式付费 allowlist、熔断、调用上限、配置和真实验收范围已由用户逐项确认；书面 spec 为 `agentwiki/docs/superpowers/specs/2026-08-04-opencode-model-fallback-cost-routing-design.md`，当前等待用户审阅，尚未开始业务代码实现。
