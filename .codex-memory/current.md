...
- v0.12.5（页面组 owner/editor 创建权限修复，1817684b）已完成 GitHub 发布；生产部署等待 SSH 凭据恢复。
...
- 本轮修改服务端页面组创建授权与回归测试并发布 v0.12.5；生产部署因 SSH `Permission denied (publickey,password)` 暂未执行。Windows 原生同步和日常 Vault 安装不属于本轮验收。
...
- v0.12.5 已提交、推送并创建 GitHub Release；生产健康接口正常，但部署需待 root@113.249.120.24 SSH 凭据恢复后再执行并做线上页面组验收。
