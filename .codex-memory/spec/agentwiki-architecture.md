...
- The remote application uses direct Node.js deployment, not Docker: user-level systemd separately supervises API, ingestion Worker and the Vite production frontend. Releases mirror source to remove stale files, explicitly regenerate Prisma Client before building, run Prisma deploy and require `/api/health` plus business smoke checks.
