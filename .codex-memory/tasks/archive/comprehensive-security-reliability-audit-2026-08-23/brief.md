...
修改、验证和发行完成。应用与 Local Sync `0.5.1` 已通过完整测试、静态门禁和空目录安装验证；GitHub `master` 已包含代码发行提交 `2700bac`，npm `latest` 已指向 `@neomei/agentwiki-local-sync@0.5.1`，生产已完成双备份、原子切换和登录态浏览器验收。
