...
- 全量 server 测试 500 通过；协议包 22 测试通过（含 3.5 fixture 公开入口断言）；typecheck/build 通过；6 个真实 PostgreSQL 集成测试通过（迁移非空、A→B→A、legacy DTO 合成、并发 pageId 唯一、并发 session 幂等唯一、真实 advance 复制/归档/指标）。
